...
class HttpProxyRepository extends PackageRepository {
  ...
  Stream<PackageVersion> versions(String package) async* {
    var versionUrl = baseUrl.resolve('/api/packages/${Uri.encodeComponent(package)}');
    ...
    if (versions != null) {
      for (var item in versions) {
        ...
        yield PackageVersion(pubspec['name'] as String, pubspec['version'] as String, pubspecString, json['time'] as int);
      }
    }
  }
  ...
  Future<PackageVersion> lookupVersion(String package, String version) {
    return versions(package).where((v) => v.packageName == package && v.versionString == version).toList().then((List<PackageVersion> versions) {
