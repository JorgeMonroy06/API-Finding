...
class ShelfPubServer {
  ...
  static final RegExp _versionRegexp = RegExp(r'^/api/packages/([^/]+)/versions/([^/]+)$');

  static final RegExp _addUploaderRegexp = RegExp(r'^/api/packages/([^/]+)/uploaders$');

  // static final RegExp _removeUploaderRegexp = RegExp(r'^/api/packages/([^/]+)/uploaders/([^/]+)$');

  static final RegExp _downloadRegexp = RegExp(r'^/packages/([^/]+)/versions/([^/]+)\.tar\.gz$');
  ...
  Future<shelf.Response> requestHandler(shelf.Request request) async {
    ...
    if (request.method == 'GET') {
      if (path == '/api/getAllPackages') {
        var list = await repository.getAllPackages();
        ...
        for (var package in list) {
          ...
          var lastName = packageVersions.last.versionString;

          result.add(DirectMessage(
            lastName,
            package,
            loadYaml(packageVersions.last.pubspecYaml),
            packageVersions.last.time,
          ));
        }
  ...
  Uri _downloadUrl(Uri url, String package, String version) {
    ...
    return url.resolve('/packages/${encode(package)}/versions/${encode(version)}.tar.gz');
  }

  Uri _readMeUrl(Uri url, String package, String version) {
    var encode = Uri.encodeComponent;
    return url.resolve('/packages/${encode(package)}/${encode(version)}/README.md');
  }

  // Upload async urls.

  Uri _finishUploadAsyncUrl(Uri url) => url.resolve('/api/packages/versions/newUploadFinish');

  // Upload custom urls.

  Uri _uploadSimpleUrl(Uri url) => url.resolve('/api/packages/versions/newUpload');
