...
class PackageVersion {
  ...
  @override
  int get hashCode => packageName.hashCode ^ versionString.hashCode ^ pubspecYaml.hashCode;

  @override
  bool operator ==(other) {
    return other is PackageVersion && other.packageName == packageName && other.versionString == versionString && other.pubspecYaml == pubspecYaml;
  }
...
class GenericProcessingException implements ClientSideProblem, Exception {
  ...
  factory GenericProcessingException.validationError(String message) => GenericProcessingException('ValidationError: $message');
...
abstract class PackageRepository {
  ...
  PackageRepository({this.local,this.baseDir});
  ...
  Future<PackageVersion> upload(Stream<List<int>> data) async => throw UnsupportedError('No upload support.');
  ...
  Future<AsyncUploadInfo> startAsyncUpload(Uri redirectUrl) async => throw UnsupportedError('No async upload support.');

  /// Finishes the upload of a package.
  Future<PackageVersion> finishAsyncUpload(Uri uri) async => throw UnsupportedError('No async upload support.');
  ...
  Future<Uri> downloadUrl(String package, String version) async => throw UnsupportedError('No download link support.');
  ...
  Future addUploader(String package, String userEmail) async => throw UnsupportedError('No uploader support.');

  /// Removes [userEmail] as an uploader from [package].
  Future removeUploader(String package, String userEmail) async => throw UnsupportedError('No uploader support.');
}
